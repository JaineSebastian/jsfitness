...
public class view_trainer extends AppCompatActivity {

    ListView li;
    SharedPreferences sh;
    String url, ip;

    String[] name,email,phone, uid,urll,t_id;
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (jsonObj.getString("status").equalsIgnoreCase("ok")) {
                                ...
                                t_id = new String[js.length()];


                                for (int i = 0; i < js.length(); i++) {
                                    JSONObject u = js.getJSONObject(i);
                                    uid[i] = u.getString("trainer_id");//dbcolumn name in double quotes
                                    name[i] = u.getString("name");
                                    email[i] = u.getString("email");
                                    phone[i] = u.getString("phone");
                                    urll[i] = u.getString("image");
                                    t_id[i] = u.getString("trainer_id");

//                                    did[i] = u.getString("did");
                                }
                                li.setAdapter(new custom_trainer(getApplicationContext(), uid, name,email,phone,urll,t_id));
                            } else {
